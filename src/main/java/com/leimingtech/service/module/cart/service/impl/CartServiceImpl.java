...
public class CartServiceImpl implements CartService {
	 ...
	 * @return
	 */
	public int saveCart(Integer goodsId,Integer memberId,Integer count,Integer goodSpecId,Integer saveType) {
		Goods goods = goodsService.findGoodById(Integer.valueOf(goodsId)); //商品信息
		...
		cart.setFirstGcId(Integer.valueOf(gcIds[0])); //将商品所在分类的一级分类的id存入购物车
		//新建一个字段存储新的规格格式
		String specInfo = "";
	 ...
	 */
	@Override
	public Cart copyGoodsToCart(Integer goodsId,Integer goodsSpecId){
		Goods goods = goodsService.findGoodById(Integer.valueOf(goodsId)); //商品信息
	 ...
	 */
	@Override
	public List<CouponMember> findCouponMemberListByCartIds(String cartIds,Integer memberId) {
		 ...
		 */
		Integer storeId = null; //店铺id
		Integer gcId = null; //商品第一级分类id
		...
		if(cartList!=null){
			for (int i=0; i<cartList.size(); i++) {
				...
				if(storeId==null){ //判断店铺id是否存在,不存在新建一个购物车按店铺id和商品所在一级分类id分组和商品所在一级分类id分组和购物车集合
				...
				}else if(storeId != cart.getStoreId()){ //判断店铺id是否更换 ,若更换新建一个购物车按店铺id和商品所在一级分类id分组和商品所在一级分类id分组和购物车集合
					...
					cartCouponVo = new CartCouponVo();
					cartLists = new ArrayList<Cart>();
				}else if(gcId != null && gcId != cart.getFirstGcId()){ 
					//判断购物车商品所在一级分类是否更换
					//若更换将购物车集合加入cartCouponVo,将分组的商品总价格加入cartCouponVo
					//将cartCouponVo加入cartCouponVoList
					//然后新建一个购物车按店铺id和商品所在一级分类id分组和购物车集合
		...
		if(couponMemberList != null){
			for(CartCouponVo couponVo:cartCouponVoList){
				for(CouponMember couponMember:couponMemberList){
					...
					if(couponVo.getStoreId()==couponMember.getStoreId()&&couponVo.getFirstGcId()==couponMember.getCouponGoodsClassId()){
